...
const OralGameResult = ({navigation}) => {
  const {
    oralQuizAttempt,
    correctAns,
    oralQuestions,
  ...
  const {processSaveOralRecords, saveInfoAlert, setSaveInfoAlert} =
  ...
  useEffect(() => {
    try {
      (oralQuizAttempt.userInfo = userProfile.username),
        console.log(oralQuizAttempt);
      setReview(oralQuizAttempt.solvedOralQuestions);
      handleGrade(oralQuestions.length, correctAns);
      setTopicEvaluation(oralQuizAttempt.solvedOralQuestions);
      let data = oralQuizAttempt.solvedOralQuestions.filter(
  ...
  const handleSaveInfo = () => {
    try {
      ...
      processSaveOralRecords(oralQuizAttempt, data2);
    } catch (err) {
      console.log(err);
    }
  };
